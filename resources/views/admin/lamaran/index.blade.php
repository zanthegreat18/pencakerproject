...
<head>
    ...
    <title>Lamaran - Admin AntiNganggur</title>
...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-gray-950 text-white p-6 space-y-4 flex flex-col justify-between">
        <div>
            ...
            <nav class="space-y-3 text-sm">
                ...
                <a href="{{ route('admin.lamaran.index') }}" class="block px-4 py-2 rounded bg-purple-800">📩 Lamaran</a>
    ...
    <main class="flex-1 bg-gradient-to-br from-purple-100 via-white to-blue-100 text-gray-900 flex flex-col">
        ...
        <div class="flex justify-between items-center bg-white px-8 py-4 shadow border-b border-gray-200">
            <div>
                <h2 class="text-lg font-semibold text-gray-800">Semua Lamaran Pencaker</h2>
            </div>
            ...
            <div class="overflow-x-auto rounded shadow-lg">
                <table class="w-full table-auto border border-gray-200 bg-white text-sm text-gray-800">
                    <thead class="bg-purple-100 text-left">
                        <tr>
                            <th class="p-3">Nama Pencaker</th>
                            <th class="p-3">Judul Lowongan</th>
                    ...
                    <tbody>
                        @forelse($lamarans as $lamaran)
                            <tr class="border-t border-gray-200 hover:bg-purple-50 transition">
                                <td class="p-3">{{ $lamaran->user->name }}</td>
                                <td class="p-3">{{ $lamaran->vacancy->judul ?? '-' }}</td>
                                <td class="p-3">{{ $lamaran->pesan ?? '-' }}</td>
                                <td class="p-3">{{ ucfirst($lamaran->status) }}</td>
                                <td class="p-3">{{ $lamaran->created_at->format('d M Y') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="p-4 text-center text-gray-400">Belum ada lamaran masuk.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
